...
void 
StoreWorker::OpContext::garbageCollectRados(
    std::unordered_map<void*, struct StoreWorker::UserCtx* const>& terms)
{

  if(!issuedOps.empty())
  {
    decltype(terms.emplace()) checkVal;
  
    for(const auto& opPair : issuedOps)
    {
      checkVal = terms.emplace(opPair.first, opPair.second);
      assert(checkVal.second);
...
{ 
  ...
  LOG(WARNING) << "StoreWorker::closeStoreWorker: ID = " 
               << static_cast<unsigned int>(Worker::getWorkerId())
               << '\n';
  ...
  //LOG(WARNING) << "StoreWorker::closeStoreWorker: "
  //             << "closed cephCtx_";
  ...
  // garbage collect the issued Rados operation contexts
  for(const auto& garbPair : termRados_)
  {
    // Ceph context is close, so it's safe
    // to delete all the user contexts
    delete garbPair.second;
  }


  termRados_.clear(); // clear the map
  ...
  //LOG(WARNING) << "StoreWorker::closeStoreWorker: ID = " 
  //             << static_cast<unsigned int>(Worker::getWorkerId())
  //             << ", notified the other thread. Exiting...";
...
StoreWorker::joinWorker()
{
  ...
  LOG(INFO) << "StoreWorker::joinWorker";
  ...
  LOG(INFO) << "StoreWorker::joinWorker: exiting joinWorker";
...
StoreWorker::processTasks()
{
  ...
  while(!Worker::done_.load(std::memory_order_relaxed))
  {
    ...
    for(auto&& tmpTask : passedTasks)
    {
       ...
       LOG(WARNING) << "StoreWorker::processTasks: ID = "
                    << static_cast<unsigned int>(Worker::getWorkerId())
                    << ", OPCode = " 
                    << static_cast<unsigned int>(tmpTask.second.opType_);
...
StoreWorker::handleActiveOps()
{
  ...
  { // process the completed operations
    ...
    LOG(WARNING) << "StoreWorker::handleActiveOps: ID = "
                 << static_cast<unsigned int>(Worker::getWorkerId())
                 << ", received some Rados Ops.";
...
StoreWorker::issueRadosWrite(StoreWorker::OpItr& iter)
{
  ...
  if(!resData)
  {
    ...
    // ensure that already issued Rados
    // will be terminated
    opIter.garbageCollectRados(termRados_);
    opIter.closeContext();
  ...
  { // Reading is still going on
    ...
    if(!(opIter.prot->doneReading()))
    {
      ...
      { // internal error
        ...
        // garbage collect the issued Rados ops
        opIter.garbageCollectRados(termRados_);
        opIter.closeContext();
        activeOps_.erase(pendItr);
...
StoreWorker::processNewWriteOp(StoreWorker::UpperRequest&& task)
{
  ...
  std::cout << "\nStoreWorker::processNewWriteOp: ID = "
            << static_cast<unsigned int>(Worker::getWorkerId())
            << std::endl;
...
StoreWorker::handleRemoteResponses()
{
  ...
  while(opIter != responses_.end())
  {
    ...
    { // handle the request and delete this item
      ...
      assert(opIter->secRes->isReady() && opIter->secRes->hasValue());

      std::cout << "\nStoreWorker::handleRemoteResponses: ID = "
                << static_cast<unsigned int>(Worker::getWorkerId())
                << ", isReady() == true" << std::endl;

      auto resOp = std::make_shared<IOResponse>(std::move(opIter->secRes->value()));
      
     

      auto res =  remProt_->handleMessage(resOp, opIter->uppReq->second,
                                          *cephCtx_);


      std::cout << "StoreWorker::handleRemoteResponse: ID = "
                << static_cast<unsigned int>(Worker::getWorkerId())
      ...
      if(res->getStatusCode() != Status::STAT_SUCCESS)
      {
        ...
        std::cout << "\nStoreWorker::handleRemoteResponse: ID = "
             << static_cast<unsigned int>(Worker::getWorkerId())
             << ", status code = " 
             << static_cast<unsigned int>(res->getStatusCode()) 
             << std::endl;
      ...
      else
      {
        std::cout << "\nStoreWorker::handleRemoteResponses: ID = "
                  << static_cast<unsigned int>(Worker::getWorkerId())
                  << ", status code = STAT_SUCCESS" << std::endl;
        processRemoteResult(opIter, res);
      }

      std::cout << "StoreWorker::handleRemoteResponse: "
                << "updating iterator." << std::endl;
...
StoreWorker::processRemoteResult(std::list<StoreWorker::WorkerContext>::iterator& resIter, std::shared_ptr<RemoteProtocol::ProtocolHandler> handler)
{
 ...
 LOG(WARNING) << "StoreWorker::processRemoteResult: ID = "
              << static_cast<unsigned int>(Worker::getWorkerId());
