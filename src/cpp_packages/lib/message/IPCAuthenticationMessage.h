...
public:
	...
	std::unique_ptr<folly::IOBuf> createMsg() override{	
		...
		memcpy(tmp,username_.c_str(),usernameLength_);
		tmp += usernameLength_;
	...
	void setUsernameLength(uint16_t usernameLength){
		usernameLength_ = usernameLength;
	};
