                                            ...
                                            const std::string& frontend_prefix)
{
  ...
  if ('\0' == req_name[0]) {
    return g_conf->rgw_sing_url_prefix == "/" ? -ERR_BAD_URL : 0;
  }

  req = req_name;

  size_t pos = req.find('/');
  if (std::string::npos != pos && g_conf->rgw_sing_url_prefix != "/") {
    bool cut_url = g_conf->rgw_sing_url_prefix.length();
    first = req.substr(0, pos);

    if (first.compare(g_conf->rgw_sing_url_prefix) == 0) {
      if (cut_url) {
        /* Rewind to the "v1/..." part. */
        next_tok(req, first, '/');
      }
    }
  } else if (req.compare(g_conf->rgw_sing_url_prefix) == 0) {
  ...
  if (! g_conf->rgw_sing_tenant_name.empty()) {
    tenant_path = "/AUTH_";
    tenant_path.append(g_conf->rgw_sing_tenant_name);
  }

  /* verify that the request_uri conforms with what's expected */
  char buf[g_conf->rgw_sing_url_prefix.length() + 16 + tenant_path.length()];
  int blen;
  if (g_conf->rgw_sing_url_prefix == "/") {
    blen = sprintf(buf, "/v1%s", tenant_path.c_str());
  } else {
    blen = sprintf(buf, "/%s/v1%s",
                   g_conf->rgw_sing_url_prefix.c_str(), tenant_path.c_str());
  }
  ...
  if (!tenant_path.empty() || g_conf->rgw_sing_account_in_url) {
    string account_name;
